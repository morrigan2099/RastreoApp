...
with st.sidebar:
    ...
    st.markdown("---")
    # EL BOTÓN QUE CAMBIA EL TAMAÑO
    modo_reporte = st.checkbox("📑 Activar Vista de Impresión (Tabloide)", value=False)
...
# ==========================================================
# CSS MAESTRO (Layout + Print Rules)
# ==========================================================
st.markdown(f"""
<style>
/* 1. LIMPIEZA INTERFAZ */
...
/* 2. REGLAS DE IMPRESIÓN (SILENCIOSAS) */
@media print {{
    @page {{ size: 17in 11in; margin: 0.5cm; }} /* Tabloide Horizontal */
    [data-testid="stSidebar"] {{ display: none !important; }} /* Adiós Sidebar */
    header, footer {{ display: none !important; }} /* Adiós Header */
...
.title-sub {{
    font-weight: 600;
    font-size: {'24px' if modo_reporte else 'clamp(16px, 5vw, 24px)'};
    line-height: 1.1;
    opacity: 0.8;
}}
...
.gallery-item {{
    ...
    padding: 4px;
    width: {'16.66%' if modo_reporte else '25%'}; /* 6 columnas en reporte, 4 normal */
}}
@media (max-width: 768px) {{
    .gallery-item {{ width: 50% !important; }} /* Móvil siempre 2 */
}}
...
# ==========================================================
# LAYOUT DINÁMICO (NORMAL vs REPORTE)
# ==========================================================

# 1. ENCABEZADO
if modo_reporte:
    # En reporte: 2 columnas (Título Izq | Stats Der)
    col_head1, col_head2 = st.columns([2, 1])
    with col_head1:
        st.markdown(html_titulo, unsafe_allow_html=True)
    with col_head2:
        # Preparamos stats para mostrar arriba a la derecha
    ...
    # Normal: Título arriba, Stats abajo del mapa
    st.markdown(html_titulo, unsafe_allow_html=True)
    # Calculamos stats igual
    for i, nombre in enumerate(sel_usuarios):
        u_data = df_f[df_f["Usuario"] == nombre]
        dist_u = 0.0
        coords = u_data[["Latitud", "Longitud"]].values.tolist()
...
# 2. MAPA
# Altura: 650px en reporte (grande), 250px normal (móvil)
alto_mapa = 650 if modo_reporte else 250
...
# 3. ESTADÍSTICAS (Solo en modo normal, en reporte ya las pusimos arriba)
if not modo_reporte:
    st.markdown("### 📊 Estadísticas")
    st.dataframe(pd.DataFrame(stats_list), use_container_width=True, hide_index=True)

# 4. GALERÍA
# En modo reporte, el título es más discreto. En normal, es H3.
...
# Botón de Cierre (Solo visible en app normal)
if not modo_reporte:
    st.markdown("---")
