...
# ==========================================================
st.set_page_config(
    page_title="Monitor 🗞️",
    layout="wide"
)

# ==========================================================
# CSS RESPONSIVE REAL (MÓVIL OK)
# ==========================================================
st.markdown("""
<style>

/* Ocultar basura Streamlit */
...
/* Botón invisible encima de imagen */
.galeria-item button {
    position: absolute;
    inset: 0;
...
with tab1:
    ...
    # ======================================================
    # GALERÍA RESPONSIVE + MODAL
    # ======================================================
    st.markdown("### 📸 Evidencias")

    df_gal = df_f[df_f["url_limpia"].notna()].reset_index(drop=True)

    if "img_modal" not in st.session_state:
        st.session_state.img_modal = None

    if not df_gal.empty:
        st.markdown('<div class="galeria">', unsafe_allow_html=True)
        ...
        for i, row in df_gal.iterrows():
            ...
            if st.button(" ", key=f"img_{i}"):
                st.session_state.img_modal = row["url_limpia"]

        st.markdown('</div>', unsafe_allow_html=True)

    # -------- MODAL --------
    if st.session_state.img_modal:
        with st.modal("📸 Evidencia"):
            st.image(st.session_state.img_modal, use_container_width=True)
            if st.button("Cerrar"):
                st.session_state.img_modal = None
