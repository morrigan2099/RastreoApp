...
/* 3. GALERÍA GRID PERSONALIZADA (A prueba de balas) */
.custom-gallery {
    display: flex;
    flex-wrap: wrap;
    margin: 0 -5px; /* Compensa el padding de las tarjetas */
}

.gallery-item {
    box-sizing: border-box;
    padding: 5px;
    /* POR DEFECTO (ESCRITORIO): 4 Columnas (25%) */
    width: 25%; 
}

/* MÓVIL: FORZAR 2 COLUMNAS (50%) */
...
.photo-box {
    ...
    overflow: hidden;
    text-align: center;
    /* Altura fija del contenedor para uniformidad */
    height: 220px; 
    display: flex;
...
.photo-box img {
    width: 100%;
    height: 190px; /* Altura reservada para la imagen */
    object-fit: contain; /* <--- ESTO EVITA EL RECORTE. Muestra la foto entera. */
    background-color: #333; /* Fondo oscuro para resaltar la foto */
}
...
with tab1:
    ...
    if sel_usuarios:
            ...
            if not u_data.empty:
                ...
                for j, row in u_data.iterrows():
                    ...
                    if ult_hito is None or (row["Hora_dt"] - ult_hito).total_seconds() >= 900:
                        folium.Marker([row["Latitud"], row["Longitud"]], icon=folium.DivIcon(html=f'<div style="font-size:18pt;">📍</div>'), popup=f"{nombre} {row['Hora']}").add_to(m)
                ...
                stats.append({"Repartidor": nombre, "Fotos": u_data['url_limpia'].notna().sum(), "Dist.": f"{dist_u:.2f} km"})
        ...
        # --- GALERÍA HTML BLINDADA ---
        st.markdown("### 📸 Evidencias")
        df_gal = df_f[df_f['url_limpia'].notna()]
        
        if not df_gal.empty:
            # Construimos HTML en una sola variable limpia
            html_gal = '<div class="custom-gallery">'
            
            for _, row in df_gal.iterrows():
                url = row['url_limpia']
                # Limpiamos nombres para evitar romper HTML
                nombre = str(row['Usuario']).split()[0].replace("<","").replace(">","")
                hora = str(row['Hora'])[:5]
                ...
                <div class="gallery-item">
                    <a href="{url}" target="_blank">
                        <div class="photo-box">
                            ...
                            <div class="photo-label">{nombre} {hora}</div>
                        </div>
                    </a>
                </div>
                """
            html_gal += '</div>'
            st.markdown(html_gal, unsafe_allow_html=True)
        else:
            st.info("Sin fotos.")
