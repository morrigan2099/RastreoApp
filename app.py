...
# --- 1. EXTRACCIÓN MAESTRA DE IMÁGENES ---
def obtener_url_final(valor):
    """Extrae la URL sin importar si es lista, texto o diccionario."""
    if not valor or str(valor).lower() in ['nan', 'none', '', '[]']:
        return None
    try:
        # Si Airtable lo manda como texto que parece lista
        if isinstance(valor, str) and valor.strip().startswith('['):
            valor = ast.literal_eval(valor)
        # Si es una lista de adjuntos
...
if not df_gps.empty:
    ...
    if not df_f.empty:
            ...
            if not u_data.empty:
                ...
                for j in range(len(u_data)):
                    ...
                    if ult_hito is None or (row['hora_dt'] - ult_hito).total_seconds() >= 900:
                        folium.Marker([row[c_lat], row[c_lon]], icon=folium.DivIcon(html=f'<div style="text-align:center;"><div style="font-size:18pt; filter: drop-shadow(1px 1px 2px black);">📍</div><div style="font-size:8pt; color:white; background:rgba(0,0,0,0.7); padding:2px 4px; border-radius:3px; font-weight:bold;">{row[c_hora][:5]}</div></div>')).add_to(m)
                        ult_hito = row['hora_dt']

                    # 3. FOTOS EN EL MAPA (Se procesan todas las filas ahora)
        ...
        if modo_reporte:
            ...
            # Recolección exhaustiva de fotos
            galeria_fotos = []
            for _, r in df_f.iterrows():
                u = obtener_url_final(r.get(c_foto))
                if u:
                    galeria_fotos.append({"url": u, "user": r[c_user], "hora": r[c_hora], "etiq": r.get(c_etiqueta, "S/E")})
